"""Этап 6, партия D часть 2: прогон V0 (дефолты) S13 (кросс-секционный моментум) на dev
2023-2025. Только портфельный режим ("Только портфельный режим" - 03_STRATEGIES.md,
"Отбор монет (V2) не применяется — пул сам по себе вход стратегии") - в отличие от
run_dev_batch_*.py здесь нет отдельного прогона "пара" по каждой монете, он бы не имел
смысла для стратегии, которая по определению ранжирует весь пул сразу.

S13 - единственная стратегия на интерфейсе PortfolioStrategy: prepare() вызывается ОДИН
раз со словарём баров всех монет сразу, а не в цикле по монетам, как для BaseStrategy
(S01-S12) в run_dev_batch_a/b/c/d.py.

Запуск: python scripts/run_dev_s13.py
"""
...
def main():
    ...
    bars_1h_by_symbol, bars_tf_by_symbol, funding_by_symbol = {}, {}, {}
    for s in symbols:
        try:
    ...
    prepared_by_symbol = strategy.prepare(bars_tf_by_symbol, ctx)  # ОДИН вызов, не цикл
