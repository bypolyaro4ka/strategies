"""Этап 6: прогон V0 (дефолты) партии D, часть 1 (S12) на dev 2023-2025.

S13 (кросс-секционный моментум) сюда не входит - требует PortfolioStrategy,
которого движок пока не поддерживает (см. JOURNAL). Будет отдельным прогоном.
...
Запуск: python scripts/run_dev_batch_d.py
"""
...
def run_one(strategy, symbols, cfg):
    ...
    for s in symbols:
        ...
        ctx = Context(params=strategy.params, protocol=cfg, funding=fnd)
    ...
    for s in bars_1h_by_symbol:
        ...
        ctx = Context(params=strat_pair.params, protocol=cfg, funding=funding_by_symbol[s])
...
def main():
    ...
    lines = ["# Партия D, часть 1 (S12) — V0 на dev 2023-01-01..2025-12-31\n",
    ...
    out_path = PROJECT_ROOT / "reports" / "dev" / "batch_D.md"
