"""Этап 5: прогон V0 (дефолты) партии A (S01, S01b, S02, S03, S04, S05) на dev 2023-2025.

Режимы «пара» (диагностика по каждой монете) и «портфель» (основной режим для рейтинга,
01_PROTOCOL.md §6). Бенчмарки Flat/Buy&Hold/Random считаются в тех же условиях.
Каждый прогон логируется в reports/registry.csv (debug=False - это НЕ отладочный запуск).

Запуск: python scripts/run_dev_batch_a.py
"""
...
def main():
    ...
    lines = ["# Партия A (тренд) — V0 на dev 2023-01-01..2025-12-31\n",
    ...
    out_path = PROJECT_ROOT / "reports" / "dev" / "batch_A.md"
