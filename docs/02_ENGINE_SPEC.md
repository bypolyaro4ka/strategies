...
| Reversal | target меняет знак, минуя 0 (напр. +1 → −1 у long_short-стратегии): один ордер закрывает старую сделку и открывает новую — в `trades` две записи, не одна незакрытая; `bars_in_trade` сбрасывается | интеграционный |
| signals_start (WF test-окно) | `run_backtest(..., signals_start=t)` не генерирует ни одного сигнального решения раньше `t` — позиции гарантированно плоские до `t`, хотя `prepare()`/индикаторы видят все бары с начала; `signals_start=None` — прежнее поведение | интеграционный |
| PortfolioStrategy dispatch | `run_backtest()` вызывает `on_bar()` `PortfolioStrategy` (S13) один раз со всеми доступными монетами сразу (`dict[symbol, row/pos]` → `dict[symbol, Decision]`), а не по одной монете независимо — ранжирование топ-k/боттом-k иначе невозможно; обычные `BaseStrategy` (S01-S12) не затронуты | интеграционный |
