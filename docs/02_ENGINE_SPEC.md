...
Для стратегий — truncation + future-poison + 2–3 ручных сценария на синтетических данных
(например, для Donchian: ряд, который пробивает максимум на известном баре).
...
- Кэшировать `prepare()` по ключу (стратегия, параметры, символ, ТФ).
